...
class IslandShipControl(var elevationTarget: Double) : ShipForcesInducer, ServerShipUser, Ticked {
    ...
    private val anchorSpeed = ModConfig.SERVER.anchorSpeed
    ...
    override fun applyForces(forcesApplier: ForcesApplier, physShip: PhysShip) {
        ...
        val buoyantFactorPerFloater = min(
            ModConfig.SERVER.floaterBuoyantFactorPerKg / 15 / mass,
            ModConfig.SERVER.maxFloaterBuoyantFactor
        )

        physShip.buoyantFactor = 1.0 + floaters * buoyantFactorPerFloater
        // Revisiting vi control code.
        ...
        if (aligning && abs(angleUntilAligned) > ALIGN_THRESHOLD) {
            ...
            val idealOmega = Vector3d(invRotationAxisAngle.x, invRotationAxisAngle.y, invRotationAxisAngle.z)
                ...
                .mul(ModConfig.SERVER.stabilizationSpeed)
        ...
        controllingPlayer?.let { player ->
            ...
            var rotationVector = Vector3d(
                ...
                if (player.leftImpulse != 0.0f)
                    (player.leftImpulse.toDouble() * ModConfig.SERVER.turnSpeed)
                else
                    -omega.y() * ModConfig.SERVER.turnSpeed,
                0.0
            )
            // rotationVector.add(player.seatInDirection.normal.toJOMLD().mul(player.leftImpulse.toDouble() * ModConfig.SERVER.turnSpeed))
            ...
            rotationVector.mul(player.leftImpulse.toDouble() * ModConfig.SERVER.turnSpeed * -1.5)
            ...
            idealForwardVel.mul(ModConfig.SERVER.baseSpeed)
            val idealCasualForwardVelInc = Vector3d(forwardVector).mul(ModConfig.SERVER.maxCasualSpeed.toDouble())
            ...
            if (player.upImpulse != 0.0f && balloons > 0)
                elevationTarget =
                    pos.y() + (
                            player.upImpulse * ModConfig.SERVER.impulseElevationRate * min(
                                elevationPower * 0.2,
                                1.5
                            )
                            )
    ...
    fun deleteIfEmpty() {
        if (helms == 0 && floaters == 0 && anchors == 0 && balloons == 0) {
            ship?.saveAttachment<IslandShipControl>(null)
        }
    }
    ...
    companion object {
        ...
        private val BALLOON_PER_MASS get() = 1 / ModConfig.SERVER.massPerBalloon
        private val NEUTRAL_FLOAT get() = ModConfig.SERVER.neutralLimit
